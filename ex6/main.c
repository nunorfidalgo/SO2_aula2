...
int _tmain(int argc, TCHAR *argv[]) {
	...
	if (RegCreateKeyEx(HKEY_LOCAL_MACHINE, TEXT("Software\\MinhaAplicacao"), 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &chave, &queAconteceu) != ERROR_SUCCESS) {
		...
		if (queAconteceu == REG_CREATED_NEW_KEY) {
			_tprintf(TEXT("Chave: HKEY_LOCAL_MACHINE\\Software\\MinhaAplicacao criada\n"));
			//Criar valor "Autor" = "Meu nome"
			RegSetValueEx(chave, TEXT("Autor"), 0, REG_SZ, (LPBYTE)TEXT("Nuno Fidalgo"), _tcslen(TEXT("Nuno Fidalgo")) * sizeof(TCHAR));
